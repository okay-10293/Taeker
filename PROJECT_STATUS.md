...
| 게시판 (목록/글쓰기/상세/댓글/좋아요) | 20% | 75% | 게시글 목록(홈/게시판/마이페이지 공통)을 카드형에서 디시인사이드 스타일의 컴팩트한 한 줄 리스트 행으로 변경 — 미리보기 텍스트 제거, 제목/댓글수/닉네임/시간/조회·좋아요를 한 줄에 배치해 스크롤 부담 감소. **신규**: 학년별 게시판(1/2/3학년) 추가 — 카테고리 바/글쓰기 선택창에 본인 학년 게시판만 동적으로 노출, 다른 학년 글은 RLS로 조회 자체 차단(`sql/grade_only_boards.sql`). 게시판 목록에서의 삭제 버튼은 아직 미구현 |
