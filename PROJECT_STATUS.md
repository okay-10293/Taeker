...
- **신규 발견 이슈(미해결)** : `comments` 테이블에 UPDATE 정책이 없어서 댓글 수정 기능 자체가
  RLS 레벨에서 막혀있음. 의도된 설계인지 확인 필요 (댓글 수정 기능을 만들 계획이면 정책 추가 필요).
...
| 게시판 (목록/글쓰기/상세/댓글/좋아요) | 25% | 65% | 변경 없음. 댓글 수정 RLS 정책 부재 확인됨(신규 이슈) |
| 마이페이지 | 10% | 60% | 변경 없음 |
...
🚧 게시글 상세 (post.html) — 실사용 데이터로 검증된 적 없음
🚧 글쓰기 (write.html) — 실제 글 등록 테스트 아직 안 함
🚧 댓글 — 실사용 검증 안 됨. **신규 확인: RLS에 UPDATE 정책이 없어 댓글 수정 자체가 불가능한 상태**
🚧 좋아요 — 실사용 검증 안 됨
...
🚧 `comments` 테이블 UPDATE 정책 부재 확인 (신규 발견, 미해결)
🚧 Leaked Password Protection 비활성 상태 (신규 발견, Supabase 대시보드에서 수동 활성화 필요)
🚧 실사용 데이터 거의 없음 — posts/comments/likes/reports 실사용 검증 여전히 필요
...
🚧 write.html에서 실제로 글을 등록해보고, post.html에서 조회수/좋아요/댓글이 정상 동작하는지 실사용 검증
🚧 auth.js 반복 회귀의 근본 원인 파악 (git 히스토리 확인 권장)
🚧 `comments` UPDATE 정책 부재 — 댓글 수정 기능을 만들지, 의도적으로 막을지 결정 필요

## 다음 작업

1. Supabase 대시보드에서 Leaked Password Protection 수동 활성화
2. 댓글 수정 기능 여부 결정 및 RLS 정책 추가(필요시)
3. 실제로 글 작성 → 조회 → 좋아요 → 댓글 → 신고까지 한 번씩 실사용 테스트
4. 마이페이지(mypage.html) 실사용 검증
5. auth.js가 왜 계속 예전 버전으로 되돌아갔었는지 원인 파악 (git 커밋 추적)
6. 관리자 페이지에 게시글/댓글 직접 삭제 UI, 전체 회원/게시글 목록 대시보드 추가
...
- **(신규, 미해결) `comments` 테이블에 UPDATE RLS 정책이 없어 댓글 수정이 원천적으로 불가능한 상태**
- **(신규, 미해결) Leaked Password Protection이 꺼져있음** (SQL로 해결 불가, 대시보드에서 수동 처리 필요)
