...
| 게시판 (목록/글쓰기/상세/댓글/좋아요) | 25% | 65% | 변경 없음 |
| 마이페이지 | 10% | 60% | 변경 없음 |
| 학교 기능 (급식/시간표/학사일정, NEIS 연동) | 15% | 70% | `school.html` 신규 구현 — 급식 조회, 학년/반별 시간표, 월별 학사일정 UI·로직 완료. **단, NEIS Open API 인증키·학교코드가 아직 등록되지 않아 실제 데이터는 표시되지 않는 상태** (관리자 패널 > 학교설정에서 등록 필요) |
