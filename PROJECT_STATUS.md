...
| 마이페이지 | 10% | 60% | 변경 없음 |
| 학교 기능 (급식/시간표/학사일정, NEIS 연동) | 15% | 93% | NEIS 학교코드 등록 완료. `school_config` SELECT 권한을 로그인 유저 전체로 확장(RLS 버그 수정). **시간표 버그 수정**: NEIS API가 공강(빈 교시)은 응답에서 아예 빼버려서 교시가 밀려 보이던 문제 해결 — 최대 교시까지 채워서 공강도 명시적으로 표시. 같은 교시에 과목이 여러 개 오는 경우(선택과목 등) 하나가 가려지지 않도록 전부 표시하도록 변경. "미술이 화학으로 보임" 현상은 이 변경으로 원인(중복 데이터 여부)이 눈에 보이게 될 것 — 계속 발생 시 추가 확인 필요 |
