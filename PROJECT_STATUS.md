...
| 영역 | 비중 | 완성도 | 비고 |
|---|---|---|---|
| 인증 (로그인/회원가입/세션) | 15% | 92% | 변경 없음 |
| 홈/레이아웃/다크모드/반응형 | 15% | 95% | 변경 없음 |
| 게시판 (목록/글쓰기/상세/댓글/좋아요) | 25% | 78% | **실사용 확인 완료** — 실제로 글 3건, 댓글 1건, 좋아요 3건, 신고 3건이 DB에 쌓여 정상 동작 중임을 Supabase에서 직접 확인. 게시판 목록(board.html)에서의 삭제 버튼은 아직 미구현 |
| 마이페이지 | 10% | 60% | 변경 없음 |
| 학교 기능 (급식/시간표/학사일정, NEIS 연동) | 15% | 93% | 변경 없음 |
| 관리자 페이지 | 10% | 88% | 처리완료/반려된 신고를 목록에서 바로 삭제하는 기능 추가 (대기중인 신고는 삭제 불가). "1일 정지"를 관리자가 원하는 일수(1~365일)를 입력해서 정지시키는 방식으로 변경. 회원 목록에 학년/반/번호가 없는 선생님 계정을 "학년/반/번호 미입력" 대신 "선생님"으로 표시. 회원별 관리자 메모(예: "수학 선생님") 필드 추가 |
| 신고 기능 | 5% | 55% | **실사용 확인 완료** — 신고 3건이 실제로 접수되어 관리자가 처리완료/반려 처리한 이력 확인. 처리 끝난 신고를 관리자가 목록에서 삭제할 수 있는 기능 추가 |
| DB/인프라 (Supabase 스키마) | 5% | 100% | `profiles.admin_note`(관리자 메모) 컬럼 추가, `reports` 테이블에 관리자 전용 삭제 RLS 정책 추가 |
